...
export async function POST(request: Request) {
  try {
    ...
    if (!Array.isArray(repos)) {
      console.error("Unexpected GitHub API response for mentor:", repos);
      return NextResponse.json(
    ...
    console.log(`Mentor ${username}'s top languages:`, languagePercentages);
    
    return NextResponse.json({ languages: languagePercentages });
  } catch (error) {
    console.error("Error fetching mentor languages:", error);
    return NextResponse.json(
      { error: "Failed to fetch mentor languages" },
      { status: 500 }
    );
  }
}